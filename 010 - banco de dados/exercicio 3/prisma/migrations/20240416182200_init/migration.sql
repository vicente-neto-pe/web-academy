...
-- CreateTable
CREATE TABLE `_categoria_subcategoria` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_categoria_subcategoria_AB_unique`(`A`, `B`),
    INDEX `_categoria_subcategoria_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `_categoria_subcategoria` ADD CONSTRAINT `_categoria_subcategoria_A_fkey` FOREIGN KEY (`A`) REFERENCES `categoria`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_categoria_subcategoria` ADD CONSTRAINT `_categoria_subcategoria_B_fkey` FOREIGN KEY (`B`) REFERENCES `categoria`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
