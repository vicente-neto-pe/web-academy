...
function firstName(fullName) {
  ...
  const blankSpace = fullName.indexOf(' ');  // Find the first space
  if (blankSpace === -1) return fullName;  // If no space is found, return the full name
  return fullName.slice(0, blankSpace);  // Return the part before the first space
}
...
function verifyStockAvailability(productType, qty) {
  ...
  if (availableStock >= qty && availableStock!==0) return true;
  else return false;
}
